...
- **Push-tap approval recovery** (hermes-agent #30 workaround) — a lost `approval.request`
  ...
  and badged-then-opened-later all covered); hydrate (`applyActivate`) **consumes** the hint and
  synthesizes a generic `ChatFeature.recoveredApprovalRequest()` card (`command: nil` + honest
  recovery `detail`) **only when the authoritative `running` is true and no real
  `pendingInteraction` arrived** — a stopped turn drops it silently (no phantom card). A real
  `approval.request` always overwrites the synthetic card AND clears the hint. This works because
  approvals have **no `request_id`** — `approval.respond` resolves a per-session FIFO, and the
  `{"resolved": n}` result surfaces the outcome: `0` → patch the optimistic status row to
  "Already handled elsewhere" (blind respond on an empty queue is a verified server no-op);
  RPC failure → `errorBanner` (no swallowed `try?`). **No push-payload change** — the
  generic-body privacy rule stands; composes with #30 (a re-surfaced real event just replaces
  the generic card). Clarify recovery is out of scope (needs the real event's `request_id`).
